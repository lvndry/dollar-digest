...
From all subagent returns, collect all candidate stories — dedupe the same event across subagents and do not cap the list. When several candidates describe the same event or topic, merge them into one candidate and combine their verified `sources` instead of keeping duplicate entries. For each candidate that might reach the final digest, answer **2–3 specific research questions** before selection, such as:
